...
from requests.exceptions import ProxyError, ConnectTimeout, SSLError, ConnectionError, ChunkedEncodingError, ReadTimeout, HTTPError
...
def fetchURL(maxAgeDays, category, output="xml", method="GET"):
    try:
        ...
        theData = {"response": list(getData(urls, method, maxAgeDays, category, header, output)) if len(urls) > 0 else []}
...
def getData(urlList: list, method: str, maxAgeDays: int, category, header, output="xml"):
    ...
    filtered_header = filter_headers(header)
    ...
    existing = {data["urlKey"]: data for data in
                db.webpages.find({"urlKey": {"$in": list(urlData.keys())}, "format": output,
                                  ...
                                  "method": {"$eq": method.upper()},
                                  "header": {"$eq": filtered_header}})}
    ...
    if len(urlKeysToObtain) > 0:
        ...
        processes = [multiprocessing.Process(target=processURLChunk, args=(
            urlKeysToObtain[x:x + chunkLen], urlData, method, output, category, maxAgeDays, filter_headers)) for x in
...
def processURLChunk(chunk, urlData, method, output, category, maxAgeDays, header):
    ...
    with ThreadPool(min(100, len(chunk))) as thread_pool:
        thread_pool.starmap(tryNTimesToGetPage,
                            [(urlKey, urlData[urlKey]["urlTuple"], method, output, category, header, maxAgeDays) for urlKey in
                             chunk])
...
def tryNTimesToGetPage(urlKey: str, urlTuple: tuple, method: str, output: str, category: str, header, maxAgeDays, tries=0,
    ...
    with pymongo.MongoClient(MONGO_LOCATION) as client:
        ...
        proxy = ph.pick()
        try:
            result = obtainPage(urlTuple, method, output, proxy, header)
            ...
            # Process data http error (404 or 500)
            return
        except:
...
def obtainPage(urlTuple: tuple, method: str, output: str, proxy: str, header):
    url, dataJson = urlTuple[0], urlTuple[1]
    downloadStartTime = datetime.now()
    requests.packages.urllib3.disable_warnings()

    with requests.request(method, url, data=json.loads(dataJson), headers=header, verify=False, stream=True,
